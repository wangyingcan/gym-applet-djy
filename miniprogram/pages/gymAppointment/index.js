...
>>>>>>> test2
  ...
  async getData() {
      ...
      if(!this.data.isReloadSystem){
        ...
        const timer = setInterval(() => {
          ...
          if (second) {
            ...
            clearInterval(timer);
            Toast.clear();    // 这会导致调用到这里的时候，所有的toast都被清除
          }
        }, 1000);
      }
    }
    ...
    // 隐藏加载提示
    wx.hideLoading();
    if (!this.data.firstEntry && !this.data.isReloadSystem) {
      Toast.success({
        message:'更新成功',
        forbidClick:true,
        selector: '#van-toast-appointment-success',
        zindex:9999
      });
    }
    // 0.9.更新数据
  ...
  async bookCourseTap(e) {
    ...
    if (selectCard.status === "active") {
      // 1.2.2.1 周卡提示
      if(selectCard.cardType == "周卡"){
        wx.showModal({
          title: '确定要预约吗？',
          content: '注意：临两小时内，周卡无取消权利',
          showCancel: true,
          cancelText: '取消',
          cancelColor: '#000000',
          confirmText: '确定',
