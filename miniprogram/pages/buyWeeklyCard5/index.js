...
Page({
  ...
  data: {
    checked: false,
    // totalFee: 29800,          // 实付金额（提交框、付款金额）
    // totalFeeStr: '298.00',      // 实付金额（字符串、商品卡片实付金额）
    // originalTotalFeeStr: '298.00',   // 月卡原价（字符串、商品卡片原价）
  ...
  async onSubmit() {
    ...
    // 1.设置商品信息
    const totalFee = 1;     // 一周五练卡实付金额
    const body = '一周五练卡购买测试';     //商品名称信息
    ...
    if (payRes.status === 'success') {
      ...
      if (result.addSuccess) {
        Toast({
          message: '购买成功',
          forbidClick: true,
          selector: '#van-toast-weeklyCard5'
        });
      } else {
        Toast({
